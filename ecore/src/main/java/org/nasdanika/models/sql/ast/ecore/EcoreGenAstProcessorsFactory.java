package org.nasdanika.models.sql.ast.ecore;
...
@EPackageNodeProcessorFactory(nsURI = AstPackage.eNS_URI)
public class EcoreGenAstProcessorsFactory {

	private Context context;
	
	public EcoreGenAstProcessorsFactory(Context context) {
		this.context = context;		
	}
	
	@EPackageNodeProcessorFactory(
			label = "SQL AST Model",
			description = "A model of SQL AST for generating code and documentation",
			actionPrototype = """
                    app-action:
                      content:
                        content-markdown:
		...
		return new EPackageNodeProcessor(config, context, prototypeProvider) {
			...
			protected void generateDiagramAndGraphActions(Collection<Label> labels, ProgressMonitor progressMonitor) {
				...
				try {
					...
					Files.writeString(new File("target/ast.drawio").toPath(), document.save(null));
				} catch (Exception e) {
					e.printStackTrace();
